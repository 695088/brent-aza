...
const SwotMatrix = () => {
  const cells = [
    { label: "Strengths", color: "#4ade80", bg: "#4ade8011", items: ["AI-powered recipe engine", "No hardware dependency", "Clear ROI ($1,500/yr saved)", "First-mover in expiration niche", "Barcode + manual entry flexibility"] },
    { label: "Weaknesses", color: "#f87171", bg: "#f8717111", items: ["Manual entry friction risk", "Incomplete barcode databases", "No network effect at launch", "Single revenue stream (freemium)", "Unknown brand equity"] },
    { label: "Opportunities", color: "#60a5fa", bg: "#60a5fa11", items: ["Grocery API auto-sync (Kroger/Instacart)", "B2B analytics for CPG brands", "EU food waste regulations driving demand", "LLM recipe quality improving rapidly", "Subscription + premium tier upsells"] },
    { label: "Threats", color: "#f59e0b", bg: "#f59e0b11", items: ["Google/Apple OS-level integration", "Supermarket apps copying feature", "LLM API cost volatility", "Low willingness-to-pay for savings apps", "User churn if onboarding is clunky"] },
  ];
    ...
    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem" }}>
      {cells.map((cell) => (
        <div key={cell.label} style={{
          ...
          <div style={{ fontFamily: "'Playfair Display', serif", fontWeight: 700, color: cell.color, fontSize: "1rem", marginBottom: "0.75rem" }}>{cell.label}</div>
...
const LeanCanvas = () => {
  const blocks = [
    { title: "Problem", color: "#f87171", content: "Households waste avg. $1,500/yr in food. No simple, affordable tool tracks expiration dates and suggests meals from what's already on hand." },
    { title: "Solution", color: "#4ade80", content: "Barcode/manual pantry logging → expiration alerts → AI meal suggestions ranked by urgency → shopping list + waste cost tracker." },
    { title: "Unique Value Proposition", color: "#a78bfa", content: "\"Stop throwing money in the trash.\" The only app that connects what's in your fridge to what's about to expire and tells you exactly what to cook tonight." },
    { title: "Unfair Advantage", color: "#f59e0b", content: "First-mover in expiration-first meal planning. AI recipe quality improving daily. Grocery API partnerships create a moat no new entrant can replicate quickly." },
    { title: "Customer Segments", color: "#60a5fa", content: "Primary: Budget-conscious households (25–45, families). Secondary: Eco-conscious millennials. Tertiary: Meal preppers and fitness trackers." },
    { title: "Key Metrics", color: "#34d399", content: "DAU/MAU ratio, items logged per user, % meals cooked from suggestions, churn rate, waste $$$ saved per user lifetime." },
    { title: "Channels", color: "#fb7185", content: "SEO content (\"reduce food waste\"), TikTok/Reels demos, food blogger partnerships, App Store/PWA, referral loop (share weekly waste report)." },
    { title: "Cost Structure", color: "#94a3b8", content: "LLM API calls (~$0.02/user/day), Supabase hosting, barcode API (Open Food Facts: free), part-time dev + marketing. Est. burn: $3–5K/month at launch." },
    { title: "Revenue Streams", color: "#fbbf24", content: "Freemium (5 items free) → Pro $4.99/mo (unlimited + AI recipes) → Family $8.99/mo. Year 1 target: 2,000 paid users = ~$120K ARR." },
  ];
